...
def measure_main_loop(audio_data, audio_sr, iterations=100):
    ...
    for iteration in range(iterations):
        ...
        while pos <= len(xh) - L:
            alpha = 0.75  # Fixed for consistent measurement
            Ha = int(Hs/alpha)
            ...
            analysis_t += time.perf_counter() - start_t
            ...
            stft_t += time.perf_counter() - start_t
            ...
            recon_t += time.perf_counter() - start_t    
        ...
        analysis_runtimes.append(analysis_t)
        stft_runtimes.append(stft_t)
        recon_runtimes.append(recon_t)
        print(f"Iteration {iteration+1}/{iterations}: {tot_runtimes[-1]:.5f} seconds")
    
    # Calculate statistics
    print(f"Analysis frame: {np.mean(analysis_runtimes) * 1000} ± {np.std(analysis_runtimes) * 1000} ms")
    print(f"STFT stage: {np.mean(stft_runtimes) * 1000} ± {np.std(stft_runtimes) * 1000} ms")
    print(f"Reconstruction: {np.mean(recon_runtimes) * 1000} ± {np.std(recon_runtimes) * 1000} ms")
    print(f"Avg total for {iterations}: {np.mean(tot_runtimes) * 1000} ± {np.std(recon_runtimes) * 1000} ms")
...
#     timings['analysis_window'] = time.time() - start
